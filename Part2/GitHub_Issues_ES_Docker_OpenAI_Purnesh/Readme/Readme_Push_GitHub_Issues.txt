...
Step5: Follow this to create commit command :
	 ...
	 For example : docker commit 6c31284597e6 purnesh11/elasticsearch_vector:latest

	 docker commit 096db03beb6c purnesh11/elasticsearch_vector:latest
...
Step6: Stop the container using the command :
	...
	For example: docker stop 6c32284597e6
	092db03beb6d
	docker stop 092db03beb6d

Step7: Push the Image to the Docker hub using the below command.

docker push YOUR-DOCKER-USERNAME/[REPOSITORY[:TAG]]

For example: docker push purnesh11/elasticsearch_vector:latest
...
Step8: We need to build the image to support multiple - platforms. 
	...
	Note: The build command requires a Dockerfile otherwise it will produce an error. This docker file contains the information about the image we need to build.
	    ...
	    For example : FROM purnesh11/elasticsearch_vector:latest

		FROM purnesh11/elasticsearch_vector:latest
...
Step8: Now we can build and push the image to docker hub, using the following command:
	...
	and then execute the below command to build a multi-platform image.
      ...
      For example: docker buildx build --platform linux/amd64,linux/arm64 -t purnesh11/elasticsearch_vector:multi --push .

	  docker buildx build --platform linux/amd64,linux/arm64 -t purnesh11/elasticsearch_vector:multi --push .
