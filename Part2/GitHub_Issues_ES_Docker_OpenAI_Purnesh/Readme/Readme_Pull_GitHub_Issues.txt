...
- Open docker-compose.yml and update the docker image name with
	...
	For example : purnesh11/elasticsearch_vector:multi

Step3: Navigate to this directory and open the terminal/cmd and run following command:

	docker-compose up -d

	This will pull the image purnesh11/elasticsearch_vector:multi from Docker hub and will run the container for it.
...
docker pull purnesh11/elasticsearch_vector:multi
...
2. docker run -d \
  ...
  -p 9200:9200 \
  --name elasticsearch \
  purnesh11/elasticsearch_vector:multi
